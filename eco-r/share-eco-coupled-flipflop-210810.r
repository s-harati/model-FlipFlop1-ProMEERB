# 2021-12-16: Prepared for sharing

# 2021-06-01:
# I revert to the version with no learning.
# Stochasticity will be in the thresholds of the U's (50 total)
# See the new code in file 5-allInOne.R
...
profit.cap = 0.1 # 210811: we set a cap for the profit that users make by harvesting 
...
if ((scenario == "SuggestVonNeumann") | (scenario == "SuggestMoore") | (scenario == "SuggestVonNeumann2") | (scenario == "SuggestMoore2") | (scenario == "Null") | (scenario == "SuggestVonNeumann3") | (scenario == "SuggestMoore3") | (scenario == "SuggestVonNeumann4") | (scenario == "SuggestMoore4")) {
  ...
  num.iters.stochastic = 1 # Null and Enforce scenarios # EDIT: Added Null to repeated runs-20210709
}
...
mask.small <-
  raster(readGDAL("Infestation_data-increment/mask_kmlps_b3.ASC", silent = T)) # added in V9p2 # small mask updated 210615 to Kamloops TSA Block 3
# DEBUGGING
########################
# TO BE REMOVED AFTER! #
...
for (iter.stochastic in 1:num.iters.stochastic) {
  ...
  for (i.start in year.calculation + ((1 - num.iters.preparation):num.iters.timesteps) * time.step) {
    ...
    if (flag.preparation) {
      ...
      outbound.revenues = rep(0, num.users)
      outbound.score = 1
    } else {
      ...
      # revenues are based on price and quantity for each user
      # price is based on previous known quantity for the total area
      # this quantity is called cut.prop.increment
      unit.price = -(160/9.0)*cut.prop.increment + (8/3.0) + unit.cost # 210810
      outbound.revenues = numeric(length = num.users)
      for (i in 1:num.users) {
        # revenue is quantity*unit.price . 
        # 210806: trying a new calculation
        outbound.revenues[i] = unit.price * sum(values(cut.zones) == i) / user.zones.size[i]
        # 210811: applying profit cap
        if (outbound.revenues[i] - outbound.costs[i] > profit.cap) {
          outbound.revenues[i] = outbound.costs[i] + profit.cap
        }
      }
      # Debugging
      cat(paste("cut.prop.increment:",cut.prop.increment,", unit.cost:",unit.cost,", unit.price:",unit.price,"\n")) 
      #
